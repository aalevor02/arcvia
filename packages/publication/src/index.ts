 ...
 *
 * ── Why this is a package and not a file in either app ──────────────────────
 * It is a contract with two ends. The studio composes one of these and stores
 * it; the published site reads it and renders nothing else. Keeping the
 * definition in the consumer meant the producer had to redeclare it, and two
 * declarations of one payload drift — silently, because a field the studio
 * stops writing is simply a field the site stops showing.
 *
 * Same reasoning as `@arcvia/viewer`, whose own description says it exists so
 * the editor and the published output "cannot drift apart".
 */
